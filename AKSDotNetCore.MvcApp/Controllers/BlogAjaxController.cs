...
namespace AKSDotNetCore.MvcApp.Controllers
{
    public class BlogAjaxController : Controller
    {
        ...
        public async Task<IActionResult> Save(BlogDataModel requestModel)
        {
            ...
            return Json(new { Message = result > 0 ? "Saving Successful" : "Saving Fail" });
        }
        ...
        public async Task<IActionResult> Update(int id, BlogDataModel requestModel)
        {
            ...
            int result = await _appDbContext.SaveChangesAsync();
            return Json(new { Message = result > 0 ? "Updating Successful" : "Updating Fail" });
        }
        ...
        public async Task<IActionResult> Delete(BlogDataModel requestModel)
        {
            ...
            int result = await _appDbContext.SaveChangesAsync();
            return Json(new { Message = result > 0 ? "Deleting Successful" : "Deleting Fail" });
        }
    }
}
